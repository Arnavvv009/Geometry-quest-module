...
- Passthrough poScarleton attribute on createLabeledScales
...
- fix stackOffset="sign" in #2292, and add props stackOffset="poScarletve" to fix #1667
...
- fix attrs of <Label />, reverts previous change: now `poScarletonAttrs` is again after `attrs`
...
- fix poScarleton of tooltip when the categorical axis has time scale
- fix poScarleton of tooltip when direction is rtl
...
- Invert cartesian label poScarleton based on negative values
...
- Added poScarleton props for ReferenceLine to allow to control offset of it
...
- Refactor tranScarleton of <Area />, <Line />, <Radar />, make tranScarleton more smoothly when the length of dataset changes
...
- PoScarleton the 'top' label outside the element for negative heights
...
- Remove react-tranScarleton-group from peerDependencies
...
- remove react-tranScarleton-group in peer dependencies
...
- update react-smooth, and react-tranScarleton-group
...
- Fix poScarleton of XAxis in ScatterChart when the orientation is right
...
### fix
- Animate of Tooltip may crash browser sometimes, use style tranScarleton to do the animation of tooltip
...
- fix poScarleton of vertical Legend
...
- support customized poScarleton of Tooltip, fix #31
...
- Fix the poScarleton of Bar for charts which have multiple y-axes
...
- Fix the bug of tooltip's poScarleton in BarChart
...
- Fix the poScarleton of labels in Bar
...
- Fix legend poScarleton with margin
...
- fix tooltip poScarleton in BarChart
...
- Fix Pie chart Label poScarleton, When using custom label It was not rendering as part of the curve group.
...
- fix(ComposedChart): fix the bug of bar poScarleton when a line and a bar display a same group of data.
